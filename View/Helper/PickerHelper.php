...
class PickerHelper extends BoostCakeFormHelper {
	...
	private $jsfiles = array(
		...
		'moment' 			=> '//cdnjs.cloudflare.com/ajax/libs/moment.js/2.6.0/moment.min.js',  // moment.js required >= 2.5.1 by datetimepicker
	...
	/**
	 * Constructor stores new paths of .
	 *
	 *
	 *
	 *
	...
	public function color($fieldName, $options = array()) {
		...
		$options['class'] = isset($options['class']) &&
			 strstr($options['class'], 'minicolors form-control') === false ? "${options['class']} minicolors form-control" : "minicolors form-control";
	...
	public function location($fieldName, $options = array()) {
		...
		$options['pickerOption'] = array_merge(!empty($options['pickerOption']) ? $options['pickerOption'] : array(), array(
		...
		echo $this->Html->scriptBlock("\$('#${divId}').locationpicker(" . preg_replace('/"*"/', '$1', json_encode($options['pickerOption'], JSON_FORCE_OBJECT |
			 JSON_PRETTY_PRINT)), self::$AIF);
	...
	public function time($fieldName, $options = array()) {
		...
		$option = array_merge(array(
			...
			'beforeInput' => '<span class="input-group-addon"><i class="add-on glyphicon glyphicon-time"></i></span>'), $options);
		$options['pickerOption'] = array_merge(!empty($option['pickerOption']) ? $option['pickerOption'] : array(), array(
			'pickDate' => false, 'pickTime' => true));
	...
	public function dateAndTime($fieldName, $options) {
		...
		$options['pickerOption'] = array_merge(array(
			'sideBySide' => true), !empty($options['pickerOption']) ? $options['pickerOption'] : array(), array(
	...
	private function generateDateTimePicker($fieldName, $options = array()) {
		...
		echo $this->Html->scriptBlock("\$(function () { \$('#${divId}').datetimepicker(" . json_encode($options['pickerOption'], JSON_FORCE_OBJECT |
			 JSON_PRETTY_PRINT) . ")});", self::$AIF);
		unset($options['pickerOption']);
		return $this->Form->input($fieldName, $options);
	}
